...
void vTaskAlarm(void *argument)
{
  ...
  for(;;)
  {
    ...
    RETURN_IF_ERROR_CODE_HEALTH(osMutexAcquire(xMutexHandle,100), &healthFlags.alarm)
    ...
    {
      RETURN_IF_ERROR_CODE_HEALTH(HAL_UART_Transmit(&huart2, (uint8_t*)"Turn on green LED\r\n", 30, 100), &healthFlags.alarm);
      HAL_GPIO_WritePin(GPIOA, GPIO_GREEN_LED, GPIO_PIN_SET);
      HAL_GPIO_WritePin(GPIOA, GPIO_RED_LED, GPIO_PIN_RESET);
    } 
    else
    {
      RETURN_IF_ERROR_CODE_HEALTH(HAL_UART_Transmit(&huart2, (uint8_t*)"Turn on red LED\r\n", 30, 100), &healthFlags.alarm);
